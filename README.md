...
| `order/index.html` | сторінка замовлення `/order` (форма Tally), українська — з сайту не злінкована, розсилається вручну |
...
Кнопки «Замовити документ» на головній скролять до форми в секції `#order` — на
`/order` ніщо із сайту не веде. У згенерованих сторінках `#order` переписується:
на `/order` → `#form`, на `/thanks` → `/#order`.
